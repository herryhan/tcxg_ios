...
@interface SameCityOrderListVC ()<UITableViewDelegate,UITableViewDataSource>
//** tableView */
...
@implementation SameCityOrderListVC
...
- (void)setupView{
    ...
    [view registerClass:[SameCityOrderTableCell class] forCellReuseIdentifier:cellId];
...
- (void)headerRefresh{
    NSString *URLString = @"delivery/list";
    ...
    [URLRequest postWithURL:URLString params:params success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        NSArray *array = [SameCityOrderModel mj_objectArrayWithKeyValuesArray:responseObject];
...
{
    SameCityOrderTableCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId forIndexPath:indexPath];
    if (cell == nil) {
        cell = [[SameCityOrderTableCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [self.mTableView cellHeightForIndexPath:indexPath model:model keyPath:@"model" cellClass:[SameCityOrderTableCell class] contentViewWidth:PX_SCREEN_WIDTH];
}
