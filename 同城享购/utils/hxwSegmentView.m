...
- (instancetype)initWithFrame:(CGRect)frame buttonName:(NSArray *)buttonName contrllers:(NSArray *)contrllers parentController:(UIViewController *)parentC {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.segmentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 40*KWidth_ScaleW)];
        ...
        self.segmentScrollV = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 40*KWidth_ScaleW, SCREEN_WIDTH, frame.size.height - 40*KWidth_ScaleW)];
        ...
        for (int i = 0; i < self.controllers.count; i++) {
            ...
            button.frame = CGRectMake(i * (SCREEN_WIDTH / self.controllers.count), 0, SCREEN_WIDTH / self.controllers.count, 40*KWidth_ScaleW);
            button.tag = i;
            [button setTitle:self.buttonName[i] forState:UIControlStateNormal];
            [button setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
            [button setTitleColor:UIColorFromRGBA(68, 195 , 34, 1) forState:UIControlStateSelected];
        ...
        // 添加下划线
        self.underLine = [[UILabel alloc] initWithFrame:CGRectMake(0, 39.5*KWidth_ScaleW, SCREEN_WIDTH / self.controllers.count, 0.5)];
      ...
      //  [self.segmentView addSubview:self.underLine];
       ...
       // [self.segmentView addSubview:self.downLine];
        ...
        self.lineImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 39.5*KWidth_ScaleW, SCREEN_HEIGHT, 0.5)];
        [self.lineImageView setImage:[UIImage imageNamed:@"lineView"]];
        [self.segmentView addSubview:self.lineImageView];
    }
